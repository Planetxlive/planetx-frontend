"use server";
import { S3Client, PutObjectCommand } from "@aws-sdk/client-s3";
import * as dotenv from "dotenv";
dotenv.config();

const s3Client = new S3Client({
  region: "ap-south-1",
  credentials: {
    accessKeyId: process.env.AWS_ACCESS_KEY_ID,
    secretAccessKey: process.env.AWS_SECRET_ACCESS_KEY,
  },
});

const bucketName = process.env.AWS_BUCKET_NAME;

export async function uploadPropertyImages(formData, numberOfImages) {
  try {
    const imageURLs = [];
    const MAX_FILE_SIZE = 10 * 1024 * 1024; // 10MB limit

    for (let i = 0; i < numberOfImages; i++) {
      const file = formData.get(`images-${i}`);
      if (!file)
        return JSON.stringify({ success: false, error: "File not provided" });

      // Check file size
      if (file.size > MAX_FILE_SIZE) {
        return JSON.stringify({
          success: false,
          error: `File size exceeds 10MB limit. Please upload smaller files.`,
        });
      }

      let imageURL = "";
      console.log(file.name);

      const fileExtension = file.name.toLowerCase().split(".")[
        file.name.toLowerCase().split(".").length - 1
      ];

      if (
        fileExtension === "png" ||
        fileExtension === "jpg" ||
        fileExtension === "jpeg"
      ) {
        const buffer = Buffer.from(await file.arrayBuffer());

        // Configure the upload parameters
        const uploadParams = {
          Bucket: bucketName,
          Key: file.name,
          Body: buffer,
          ContentType: `image/${fileExtension}`,
        };

        // Upload the file
        const data = await s3Client.send(new PutObjectCommand(uploadParams));

        console.log("Upload successful: ", data);

        const signedUrl = `https://${process.env.AWS_BUCKET_NAME}.s3.${process.env.AWS_REGION}.amazonaws.com/${file.name}`;
        imageURL = signedUrl;

        imageURLs.push(imageURL);
      } else {
        return JSON.stringify({
          success: false,
          error: "Only .png, .jpg, .jpeg files are allowed",
        });
      }
    }

    return JSON.stringify({
      success: true,
      message: "File upload to object store",
      imageURLs: imageURLs,
    });
  } catch (error) {
    console.error("Error uploading Image:", error);
    return JSON.stringify({ success: false, error: error.message });
  }
}

export async function uploadBlogImage(file) {
  console.log("uploading app")
  try {
    const MAX_FILE_SIZE = 10 * 1024 * 1024; // 10MB limit
    
    if (file.size > MAX_FILE_SIZE) {
      return JSON.stringify({
        success: false,
        error: `File size exceeds 10MB limit. Please upload smaller files.`,
      });
    }

    console.log(file.name);
    const fileExtension = file.name.toLowerCase().split(".")[
      file.name.toLowerCase().split(".").length - 1
    ];

    if (
        fileExtension === "png" ||
        fileExtension === "jpg" ||
        fileExtension === "jpeg"
      ) {
        const buffer = Buffer.from(await file.arrayBuffer());
        // Configure the upload parameters
        const uploadParams = {
          Bucket: bucketName,
          Key: file.name,
          Body: buffer,
          ContentType: `image/${fileExtension}`,
        };

        // Upload the file
        const data = await s3Client.send(new PutObjectCommand(uploadParams));

        console.log("Upload successful: ", data);

        const signedUrl = `https://${process.env.AWS_BUCKET_NAME}.s3.${process.env.AWS_REGION}.amazonaws.com/${file.name}`;
        console.log(signedUrl)
        return signedUrl;
      } else {
        return JSON.stringify({
          success: false,
          error: "Only .png, .jpg, .jpeg files are allowed",
        });
      }

  } catch (error) {
    console.error("Error uploading Image:", error);
    return "";
  }
}

export async function uploadPropertyVideo(formData) {
  try {
    const file = formData.get("video");
    if (!file) {
      console.error("No video file provided in formData");
      return JSON.stringify({ success: false, error: "File not provided" });
    }

    const MAX_FILE_SIZE = 50 * 1024 * 1024; // 50MB limit for videos
    
    if (file.size > MAX_FILE_SIZE) {
      console.error(`Video size ${file.size} exceeds limit of ${MAX_FILE_SIZE}`);
      return JSON.stringify({
        success: false,
        error: `Video size exceeds 50MB limit. Please upload a smaller video.`,
      });
    }

    let videoURL = "";
    console.log("Processing video file:", file.name);
    const uploadName = file.name.toLowerCase().replaceAll(" ", "_");

    const fileExtension = file.name.toLowerCase().split(".")[
      file.name.toLowerCase().split(".").length - 1
    ];

    if (fileExtension === "mp4") {
      try {
        const buffer = Buffer.from(await file.arrayBuffer());
        console.log("Buffer created successfully, size:", buffer.length);

        // Configure the upload parameters
        const uploadParams = {
          Bucket: bucketName,
          Key: uploadName,
          Body: buffer,
          ContentType: `video/${fileExtension}`,
        };

        console.log("Attempting S3 upload with params:", {
          Bucket: bucketName,
          Key: uploadName,
          ContentType: `video/${fileExtension}`,
        });

        // Upload the file
        const data = await s3Client.send(new PutObjectCommand(uploadParams));
        console.log("S3 upload successful:", data);

        const signedUrl = `https://${process.env.AWS_BUCKET_NAME}.s3.${process.env.AWS_REGION}.amazonaws.com/${uploadName}`;
        videoURL = signedUrl;
        console.log("Generated video URL:", videoURL);
      } catch (uploadError) {
        console.error("Error during video upload process:", uploadError);
        return JSON.stringify({ 
          success: false, 
          error: `Failed to upload video: ${uploadError.message}` 
        });
      }
    } else {
      console.error("Invalid file extension:", fileExtension);
      return JSON.stringify({
        success: false,
        error: "Only .mp4 files are allowed",
      });
    }

    return JSON.stringify({
      success: true,
      message: "File upload to object store",
      videoURL: videoURL,
    });
  } catch (error) {
    console.error("Error in uploadPropertyVideo:", error);
    return JSON.stringify({ 
      success: false, 
      error: `Video upload failed: ${error.message}` 
    });
  }
}
